...
namespace SignageHADO
{
    ...
    public class TimelineAnimationHandler : MonoBehaviourCustom
    {
            ...
            try
            {
                if (duration > 0)
                {
                    ...
                    while (_onResetPositionToken != null)
                    {
                        ...
                        slerp = Mathf.Clamp01(elapsedTime / duration);
                        position = Vector3.Slerp(transform.localPosition, Vector3.zero, slerp);
                        rotation = Quaternion.Slerp(transform.localRotation, Quaternion.identity, slerp);
